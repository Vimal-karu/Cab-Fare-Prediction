##########Cab fare pridiction#############################
setwd("D:/Data Scientist/Project/Car fare prediction")
getwd()
...
##As missing value is present in fare_amount that is our target variable we remove the rows of presence of missing value in fare_amount 
train = DropNA(train, fare_amount, message = TRUE )
library(tidyr)
train = drop_na(train, fare_amount)
...
av_test = aov(fare_amount~passenger_count+Day+hour+week_days+Month+Year , data = train)
summary(av_test)

numeric_index = sapply(train,is.numeric)
numeric_data = train[,numeric_index]
numeric_data = subset(numeric_data,select = -c(week_days))
